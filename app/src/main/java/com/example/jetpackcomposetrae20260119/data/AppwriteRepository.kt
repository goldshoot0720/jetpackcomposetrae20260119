    ...
    suspend fun getSubscriptions(): List<Subscription> = withContext(Dispatchers.IO) {
        try {
            ...
            while (true) {
                ...
                Log.d("AppwriteRepository", "Fetched ${response.documents.size} documents (offset=$offset)")
                val batch = response.documents.map { doc ->
                    Subscription.fromDocument(doc)
                }
                allSubscriptions.addAll(batch)

                if (response.documents.size < limit) break
                offset += limit
            }

            Log.d("AppwriteRepository", "Total subscriptions fetched: ${allSubscriptions.size}")
            allSubscriptions
        } catch (e: Exception) {
